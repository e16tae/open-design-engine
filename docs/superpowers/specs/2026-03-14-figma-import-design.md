...
pub enum FigmaNodeType {
    ...
    TABLE,
    TABLE_CELL,
    // FigJam types (skipped during import)
    STICKY,
    SHAPE_WITH_TEXT,
    CONNECTOR,
}
...
pub struct FigmaNode {
    ...
    pub stroke_cap: Option<String>,    // NONE | ROUND | SQUARE
...
pub struct FigmaVariableCollection {
    pub id: String,
    pub name: String,
    pub modes: Vec<FigmaVariableMode>,  // actually a Map in API, normalized here
...
| `IMAGE` | `Image` | Via `imageRef` → download → embed |
| FigJam types | — | Skipped with warning |
...
| `IMAGE` | `Paint::ImageFill` | `scaleMode` → `ImageFillMode`, download via API |
| `EMOJI` | — | Skipped |
...
|---|---|---|
| `DROP_SHADOW` | `Effect::DropShadow` | Direct mapping |
| `INNER_SHADOW` | `Effect::InnerShadow` | Direct mapping |
| `LAYER_BLUR` | `Effect::LayerBlur` | `radius` maps to `radius` |
| `BACKGROUND_BLUR` | `Effect::BackgroundBlur` | `radius` maps to `radius` |
...
| `fontWeight` | `font_weight` | f32 → u16 |
...
| `VariableCollection.modes` | `Vec<Mode>` | mode_id → Mode.id |
| `VariableCollection.defaultModeId` | `default_mode` | |
| `Variable` | `Token` | |
...
| `Variable.resolvedType: BOOLEAN` | `TokenValue::Number` | true→1.0, false→0.0 (ODE에 Boolean 없음) |
| `VariableAlias` | `TokenResolve::Alias` | id → collection_id + token_id 매핑 |
| `Variable.valuesByMode` | `Token.values` | mode별 값 매핑 |
...
상대 좌표 명령어(`m`, `l`, `q`, `c`, `z`)도 지원하며, 절대 좌표로 변환한다.
